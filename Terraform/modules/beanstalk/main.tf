locals {
  application_name = "${var.team_name}-backend-app-${var.environment}"
...
#   name        = "${local.application_name}-${var.environment}-v1"
#   application = aws_elastic_beanstalk_application.this.name
#   description = "Spring Boot application version deployment"
...
#   # Security groups for the instances (under autoscaling:launchconfiguration)
#   setting {
#     namespace = "aws:autoscaling:launchconfiguration"
#     name      = "SecurityGroups"
#     value     = join(",", var.security_group_ids)
